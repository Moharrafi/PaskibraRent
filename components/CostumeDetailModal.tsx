...
import { X, Check, ShieldCheck, Ruler, Box, ShoppingBag, ChevronRight, Star, ArrowRight, CalendarClock, Bell } from 'lucide-react';
        ...
        <div className="w-full md:w-1/2 bg-slate-50 p-4 md:p-6 flex flex-col shrink-0 relative overflow-y-auto md:overflow-hidden">
          ...
          >
            <img
              ...
              alt={costume.name}
              className={`w-full h-full object-cover object-top transition-transform duration-700 ease-out ${isZoomed ? 'scale-110' : 'scale-100'} ${isFullyBooked ? 'grayscale contrast-75' : ''}`}
            />
          </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar shrink-0 justify-center md:justify-start">
            {galleryImages.map((img, idx) => (
              <button
                ...
                className={`relative flex-shrink-0 w-16 h-16 md:w-16 md:h-16 rounded-xl overflow-hidden border-2 transition-all duration-300 ${activeImageIndex === idx
          ...
          <div className="p-4 md:p-6 bg-white border-t border-slate-100 shrink-0 z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isInCart}
                className={`flex-1 py-3 md:py-3.5 px-4 md:px-6 rounded-xl font-bold text-white flex items-center justify-center gap-2 shadow-lg transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] text-sm md:text-base ${isInCart
