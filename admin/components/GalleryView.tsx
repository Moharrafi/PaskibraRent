...
const GalleryView: React.FC<GalleryViewProps> = ({ onShowToast }) => {
  ...
  const handleConfirmDelete = () => {
    if (deleteConfirmId) {
      const updated = storage.deleteGalleryItem(deleteConfirmId);
      setItems(updated);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updated = storage.saveGalleryItem(newItem);
    setItems(updated);
    setIsModalOpen(false);
    onShowToast(editingId ? 'Foto berhasil diperbarui' : 'Foto berhasil ditambahkan', 'success');
  };
